...
function New-DSCConfiguration {
    ...
    if ($configParams.Count -eq 0) {
        ...
        if ($PolicyName -like "*Security Options - System objects*") {
            ...
            $dscResources += "        }`r`n`r`n"
        } elseif ($PolicyName -like "*System Audit Policies - Privilege Use*") {
            # Add hardcoded Privilege Use audit configuration
            if ($dscModules -notcontains "AuditPolicyDsc") { $dscModules += "AuditPolicyDsc" }
            $dscResources += "        # Hardcoded Privilege Use audit configuration`r`n"
            $dscResources += "        AuditPolicySubcategory 'SensitivePrivilegeUse_Success'`r`n"
            $dscResources += "        {`r`n"
            $dscResources += "            Name = 'Sensitive Privilege Use'`r`n"
            $dscResources += "            AuditFlag = 'Success'`r`n"
            $dscResources += "            Ensure = 'Present'`r`n"
            $dscResources += "        }`r`n`r`n"
            $dscResources += "        AuditPolicySubcategory 'SensitivePrivilegeUse_Failure'`r`n"
            $dscResources += "        {`r`n"
            $dscResources += "            Name = 'Sensitive Privilege Use'`r`n"
            $dscResources += "            AuditFlag = 'Failure'`r`n"
            $dscResources += "            Ensure = 'Present'`r`n"
            $dscResources += "        }`r`n`r`n"
            $dscResources += "        AuditPolicySubcategory 'NonSensitivePrivilegeUse_Success'`r`n"
            $dscResources += "        {`r`n"
            $dscResources += "            Name = 'Non Sensitive Privilege Use'`r`n"
            $dscResources += "            AuditFlag = 'Success'`r`n"
            $dscResources += "            Ensure = 'Absent'`r`n"
            $dscResources += "        }`r`n`r`n"
            $dscResources += "        AuditPolicySubcategory 'NonSensitivePrivilegeUse_Failure'`r`n"
            $dscResources += "        {`r`n"
            $dscResources += "            Name = 'Non Sensitive Privilege Use'`r`n"
            $dscResources += "            AuditFlag = 'Failure'`r`n"
            $dscResources += "            Ensure = 'Absent'`r`n"
