...
function New-DSCConfiguration {
    ...
    foreach ($param in $Parameters) {
        ...
        if ($PolicyName -like "*System Audit Policies*") {
            ...
            # Transform audit flag values for AuditPolicyDsc compatibility
            $dscResources += "            AuditFlag = switch (`$$($param.Name)) {`r`n"
            $dscResources += "                'No Auditing' { 'None' }`r`n"
            $dscResources += "                'Success and Failure' { 'Success', 'Failure' }`r`n"
            $dscResources += "                'Success' { 'Success' }`r`n"
            $dscResources += "                'Failure' { 'Failure' }`r`n"
