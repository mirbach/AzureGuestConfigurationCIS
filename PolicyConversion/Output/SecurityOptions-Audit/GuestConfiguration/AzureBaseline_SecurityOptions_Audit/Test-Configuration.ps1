...
Write-Host "Testing AzureBaseline_SecurityOptions_Audit configuration..." -ForegroundColor Green
...
try {
    ...
    . ".\AzureBaseline_SecurityOptions_Audit.ps1"
    ...
    AzureBaseline_SecurityOptions_Audit @configParams -OutputPath ".\TestOutput" -WarningAction SilentlyContinue | Out-Null
...
Write-Host "  }" -ForegroundColor Cyan
  New-AzPolicyAssignment -Name 'audit-policy' \
    -PolicyDefinition (Get-AzPolicyDefinition -Name 'GG-Deploy-SecurityOptions_Audit') \
...
Write-Host "    --policy 'GG-Deploy-SecurityOptions_Audit' \" -ForegroundColor Cyan
