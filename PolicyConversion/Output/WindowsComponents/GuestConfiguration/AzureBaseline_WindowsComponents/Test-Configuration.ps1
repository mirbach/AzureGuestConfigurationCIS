...
Write-Host "Testing AzureBaseline_WindowsComponents configuration..." -ForegroundColor Green
...
try {
    ...
    . ".\AzureBaseline_WindowsComponents.ps1"
    ...
    AzureBaseline_WindowsComponents @configParams -OutputPath ".\TestOutput" -WarningAction SilentlyContinue | Out-Null
...
Write-Host "  }" -ForegroundColor Cyan
  New-AzPolicyAssignment -Name 'audit-policy' \
    -PolicyDefinition (Get-AzPolicyDefinition -Name 'GG-Deploy-WindowsComponents') \
...
Write-Host "    --policy 'GG-Deploy-WindowsComponents' \" -ForegroundColor Cyan
