...
Write-Host "Testing AzureBaseline_SecurityOptions_Accounts configuration..." -ForegroundColor Green
Write-Host "Assignment Type: $AssignmentType" -ForegroundColor Yellow

# Display parameters
Write-Host "Parameters:" -ForegroundColor Yellow
Write-Host "  AccountsGuestAccountStatus: $AccountsGuestAccountStatus" -ForegroundColor Cyan
...
try {
    ...
    . ".\AzureBaseline_SecurityOptions_Accounts.ps1"
    
    # Create parameters hashtable
    $configParams = @{
        'AccountsGuestAccountStatus' = $AccountsGuestAccountStatus
    }
    
    Write-Host "Compiling configuration..." -ForegroundColor Yellow
    
    # Compile the configuration
    AzureBaseline_SecurityOptions_Accounts @configParams -OutputPath ".\TestOutput" -WarningAction SilentlyContinue | Out-Null
...
Write-Host "    'AccountsGuestAccountStatus' = '0'" -ForegroundColor Cyan
Write-Host "  }" -ForegroundColor Cyan
Write-Host "  New-AzPolicyAssignment -Name 'audit-policy' \" -ForegroundColor Cyan
Write-Host "    -PolicyDefinition (Get-AzPolicyDefinition -Name 'GG-Deploy-SecurityOptions_Accounts') \" -ForegroundColor Cyan
Write-Host "    -Scope '/subscriptions/your-sub-id' \" -ForegroundColor Cyan
Write-Host "    -PolicyParameterObject `$params" -ForegroundColor Cyan
...
Write-Host "    --policy 'GG-Deploy-SecurityOptions_Accounts' \" -ForegroundColor Cyan
Write-Host "    --scope '/subscriptions/your-sub-id' \" -ForegroundColor Cyan
