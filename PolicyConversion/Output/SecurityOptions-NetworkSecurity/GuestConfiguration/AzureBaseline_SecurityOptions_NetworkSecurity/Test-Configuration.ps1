...
Write-Host "Testing AzureBaseline_SecurityOptions_NetworkSecurity configuration..." -ForegroundColor Green
...
try {
    ...
    . ".\AzureBaseline_SecurityOptions_NetworkSecurity.ps1"
    ...
    AzureBaseline_SecurityOptions_NetworkSecurity @configParams -OutputPath ".\TestOutput" -WarningAction SilentlyContinue | Out-Null
...
Write-Host "  }" -ForegroundColor Cyan
  New-AzPolicyAssignment -Name 'audit-policy' \
    -PolicyDefinition (Get-AzPolicyDefinition -Name 'GG-Deploy-SecurityOptions_NetworkSecurity') \
...
Write-Host "    --policy 'GG-Deploy-SecurityOptions_NetworkSecurity' \" -ForegroundColor Cyan
