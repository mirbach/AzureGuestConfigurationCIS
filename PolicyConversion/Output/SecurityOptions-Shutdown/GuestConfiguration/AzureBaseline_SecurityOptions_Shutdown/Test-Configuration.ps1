...
Write-Host "Testing AzureBaseline_SecurityOptions_Shutdown configuration..." -ForegroundColor Green
...
try {
    ...
    . ".\AzureBaseline_SecurityOptions_Shutdown.ps1"
    ...
    AzureBaseline_SecurityOptions_Shutdown @configParams -OutputPath ".\TestOutput" -WarningAction SilentlyContinue | Out-Null
...
Write-Host "  }" -ForegroundColor Cyan
  New-AzPolicyAssignment -Name 'audit-policy' \
    -PolicyDefinition (Get-AzPolicyDefinition -Name 'GG-Deploy-SecurityOptions_Shutdown') \
...
Write-Host "    --policy 'GG-Deploy-SecurityOptions_Shutdown' \" -ForegroundColor Cyan
