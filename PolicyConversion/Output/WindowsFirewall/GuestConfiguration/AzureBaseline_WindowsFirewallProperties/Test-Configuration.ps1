...
Write-Host "Testing AzureBaseline_WindowsFirewallProperties configuration..." -ForegroundColor Green
...
try {
    ...
    . ".\AzureBaseline_WindowsFirewallProperties.ps1"
    ...
    AzureBaseline_WindowsFirewallProperties @configParams -OutputPath ".\TestOutput" -WarningAction SilentlyContinue | Out-Null
...
Write-Host "  }" -ForegroundColor Cyan
  New-AzPolicyAssignment -Name 'audit-policy' \
    -PolicyDefinition (Get-AzPolicyDefinition -Name 'GG-Deploy-WindowsFirewallProperties') \
...
Write-Host "    --policy 'GG-Deploy-WindowsFirewallProperties' \" -ForegroundColor Cyan
